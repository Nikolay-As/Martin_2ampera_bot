...
io.on('connection', function(socket) {
    ...
    socket.on('processed', function(result) {
        bot.telegram.sendMessage(result[1],'Заявка номер '+result[0]+' обработана')
    });


    socket.on('error', function(result) {
        bot.telegram.sendMessage(result[1],'Заявка номер '+result[0]+' не обработана(ошибка в заявке), напиши админу')
    });
...
app.post('/processed', (req, res) => {
        ...
        bot.telegram.sendMessage(req.body.id_user,'Заявка номер '+req.body.id+' обработана')
        console.log("nen z")
})
...
Check_status_1.leave(ctx=>{
    ...
    pool.query(sql, data   , function(err, results) {
        if(err) {
            ...
            if(results.length==0){
                ctx.reply('Такой заявки нет,либо не вы создавали ее ( только автор заявки может посмотреть ее статус )',menu_keyboard )
            }else {
                ctx.reply(`Тип заявки: ${results[0].type}\nСтатус заявки: ${results[0].status}`,menu_keyboard)
            }
        }
...
bot.hears('💳Узнать номер карты', ctx=>ctx.reply('2202 2026 2349 5888 Николай П'),menu_instructions)
